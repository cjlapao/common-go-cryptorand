...
LAST_RELEASE_MERGED_AT=$(gh pr view $LAST_RELEASE_PR --repo cjlapao/common-go-cryptorand --json mergedAt | jq -r '.mergedAt')

CHANGELIST=$(gh pr list --repo cjlapao/common-go-cryptorand --base main --state merged --json title --search "merged:>$LAST_RELEASE_MERGED_AT -label:no-release")
